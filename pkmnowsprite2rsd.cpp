...
u8             image_data_tmp[ 256 * 256 * 20 / 4 + 100 ] = { 0 };
...
int main( int p_argc, char** p_argv ) {
    ...
    uint16_t       NUM_FRAMES = 0, HEIGHT = 32, WIDTH = 32, THRESHOLD = 10, SCALE = 2;
    ...
    printf( "[%s] %u %u %u\n", p_argv[ 1 ], HEIGHT, WIDTH, NUM_FRAMES );
            ...
            for( size_t x = 0; x < WIDTH; x++ ) {
                ...
                if( !palidx.count( conv_color ) ) {
                    ...
                    if( min_del < THRESHOLD && col + start ) {
                        ...
                        fprintf( stderr, " adding color \x1b[48;2;%u;%u;%um%4hx\x1b[0;00m\n",
                                 red( conv_color ), blue( conv_color ), green( conv_color ),
                                 conv_color & 0x7fff );
    ...
    // check for white space, trim image
    bool trim = false;
    for( u8 i = 0; i < 2 && WIDTH > 16; ++i ) {
        bool non0found = false;
        for( size_t frame = 0; frame < NUM_FRAMES; ++frame )
            for( size_t y = 0; y < HEIGHT; y++ ) {
                for( size_t x = 0; x < WIDTH / 4; x++ ) {
                    non0found |= !!image_data[ WIDTH * HEIGHT * frame + y * WIDTH + x ];
                }
                for( size_t x = 3 * WIDTH / 4; x < WIDTH; x++ ) {
                    non0found |= !!image_data[ WIDTH * HEIGHT * frame + y * WIDTH + x ];
                }
            }

        if( !non0found ) { // actually trim image
            trim = true;
            fprintf( stderr, "[%s] Trimming horizontally down to %hu.\n", p_argv[ 1 ], WIDTH / 2 );
            auto nwidth = WIDTH >> 1;
            for( size_t frame = 0; frame < NUM_FRAMES; ++frame )
                for( size_t y = 0; y < HEIGHT; y++ ) {
                    for( size_t x = WIDTH / 4, x2 = 0; x < 3 * WIDTH / 4; x++, x2++ ) {
                        image_data_tmp[ nwidth * HEIGHT * frame + y * nwidth + x2 ]
                            = image_data[ WIDTH * HEIGHT * frame + y * WIDTH + x ];
                    }
                }
            WIDTH >>= 1;
            numTiles >>= 1;
            std::memcpy( image_data, image_data_tmp, WIDTH * HEIGHT * NUM_FRAMES );
        }
    }

    for( u8 i = 0; i < 2 && HEIGHT > WIDTH; ++i ) {
        bool non0found = false;
        for( size_t frame = 0; frame < NUM_FRAMES; ++frame )
        ...
        if( !non0found ) { // actually trim image
            trim = true;
            fprintf( stderr, "[%s] Trimming vertically down to %hu.\n", p_argv[ 1 ], HEIGHT / 2 );
            auto nheight = HEIGHT >> 1;
            for( size_t frame = 0; frame < NUM_FRAMES; ++frame )
                for( size_t y = HEIGHT / 2, y2 = 0; y < HEIGHT; y++, y2++ ) {
                    for( size_t x = 0; x < WIDTH; x++ ) {
                        image_data_tmp[ WIDTH * nheight * frame + y2 * WIDTH + x ]
                            = image_data[ WIDTH * HEIGHT * frame + y * WIDTH + x ];
                    }
                }
            HEIGHT >>= 1;
    ...
    } else if( !sscanf( p_argv[ 1 ], "%03hu", &pkmnidx ) ) {
        // Output to cwd
        fout = fopen( ( rspth + ( genraw ? ".raw" : ".rsd" ) ).c_str( ), "wb" );
    } else {
        char buffer[ 50 ];
        snprintf( buffer, 40, "%02hhu/", pkmnidx / MAX_ITEMS_PER_DIR );
        fs::create_directories( buffer );
